...
static const volatile float vone = 1.0f,  vzero = 0.0f;
...
__ieee754_j0f(float x)
{
	 ...
	 */
		if(ix>0x58000000) z = (invsqrtpi*cc)/gm_sqrtf(x); /* |x|>2**49 */
		else {
		    u = pzerof(x); v = qzerof(x);
		    z = invsqrtpi*(u*cc-v*ss)/gm_sqrtf(x);
		}
		return z;
	}
...
__ieee754_y0f(float x)
{
	 ...
	 */
                ...
                if(ix>0x58000000) z = (invsqrtpi*ss)/ gm_sqrtf(x); /* |x|>2**49 */
                else {
                    u = pzerof(x); v = qzerof(x);
                    z = invsqrtpi*(u*ss+v*cc)/ gm_sqrtf(x);
                }
                return z;
	}
